"""
NSV Controller v11 — cleaned & tightened
Changes from v10:
- Image saving respects camera selection.
- Metrics keep blanks for deselected/undefined sensors (no fake zeros).
- Preview vs. recording resolution split (recording ≥480p).
- True blue header (BGR) & minor UI polish.
- Graph data bounded with deque to avoid unbounded growth.
- Safer VideoWriter open with checks; CAP_DSHOW→CAP_ANY fallback.
- Thread shutdown joins; small refactors & dead-code trims.
"""
...
RECORD_RES  = (848, 480)     # ≥480p; bump to (1280, 720) for 720p
CAM_FPS     = 25
TOF_HZ      = 100
TEN_M_METERS= 10.0
TEN_M_KM    = 0.01
...
class CameraWorker(threading.Thread):
    def __init__(self, cam_index, out_queue, get_writer, get_overlay_ctx,
        ...
        self.pw, self.ph = preview_size
        self.rw, self.rh = record_size
    ...
    def draw_header(self, frame, ctx):
        # Ensure preview size
        if frame.shape[0] != self.ph or frame.shape[1] != self.pw:
    ...
    def _open_cap(self):
        ...
        if not (self.cap and self.cap.isOpened()):
            self.cap = cv2.VideoCapture(self.cam_index)  # CAP_ANY fallback
        ...
        try:
            if not self._open_cap():
                while not self.stop_event.is_set():
                    frame = make_testcard(self.pw, self.ph, f"{CAM_NAMES.get(self.cam_index, f'CAM {self.cam_index}')} NOT FOUND")
                    frame = self.draw_header(frame, self.get_overlay_ctx())
                    self.latest_frame = frame
                    self.out_queue.put((self.cam_index, frame))
                    writer = self.get_writer(self.cam_index)
                    if writer is not None:
                        writer.write(cv2.resize(frame, (self.rw, self.rh)))
                    time.sleep(1.0/CAM_FPS)
                return

            # Prefer native capture size; we resize ourselves
            while not self.stop_event.is_set():
                ok, frame = self.cap.read()
                if not ok:
                    frame = make_testcard(self.pw, self.ph, f"{CAM_NAMES.get(self.cam_index, f'CAM {self.cam_index}')} ERROR")
                frame = self.draw_header(frame, self.get_overlay_ctx())
                self.latest_frame = frame
                self.out_queue.put((self.cam_index, frame))
                writer = self.get_writer(self.cam_index)
...
class NSVApp(ctk.CTk):
    ...
    def build_graphs(self, parent):
        ...
        self.graph_data = [deque(maxlen=600) for _ in range(6)]  # ~6 km @10 m bins
    ...
    def capture_images(self, chainage_label, lat=None, lon=None, alt=None):
        ...
        stamp = ts_for_path(); ctx = self.get_overlay_context()
        for cam_idx, worker in self.cameras.items():
            # Respect camera selection for image capture
        ...
        try:
            ...
            while True:
                ...
                if i < len(self.cam_labels):
                    lbl = self.cam_labels[i]; lbl.configure(image=imgtk); lbl.image = imgtk
        except queue.Empty:
            pass
        self.after(30, self.poll_cam_frames)
