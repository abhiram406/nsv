"""
NSV Controller v18 — optimized for low-latency camera streams with updated header

Key features:
- Reduced header height (20% of frame) with updated column layout and smaller font size.
- Header columns: 
  - Col1: Project ID, Survey Location, NH Number, Old NH Number, Section Code, Direction
  - Col2: Latitude, Longitude, Altitude, Distance, Chainage, Speed, LRP
  - Col3: Date (DD-MM-YYYY), Time (HH:MM:SS)
- Preview windows show raw camera feed without header or text.
- GUI opens maximized on startup.
- Optimized RTSP streams with low-latency settings (UDP, reduced buffering).
- CameraWorker and poll_cam_frames optimized to reduce queue congestion and lag.
- Right panel shows Speed, Distance, Chainage, Latitude, Longitude, and Altitude.
- 6 sensors selected by default, up to 13 supported.
- Project setup fields pre-filled.
- Buttons in two rows, with speed progress bar.
"""
...
CAM_URLS = [
    "rtsp://192.168.1.28:554/stream",  # Front - replace with actual URL
    "rtsp://192.168.1.28:554/stream",  # Left - replace with actual URL
    "rtsp://192.168.1.28:554/stream",  # Right - replace with actual URL
    "rtsp://192.168.1.28:554/stream"   # Back - replace with actual URL
]
...
HEADER_LINES = 6             # Lines per column (max for Col1)
...
def draw_header_to_size(frame, ctx, size):
    ...
    cols = [col1, col2[:6], col3]  # Limit col2 to 6 lines to match col1
        ...
        try:
            if not self._open_cap():
                while not self.stop_event.is_set():
                    ...
                    preview = cv2.resize(make_testcard(self.preview_size[0], self.preview_size[1], "CAM NOT FOUND"), self.preview_size)
                    record = draw_header_to_size(make_testcard(self.record_size[0], self.record_size[1], "CAM NOT FOUND"), ctx, self.record_size)
                    self.latest_preview_frame = cv2.cvtColor(preview, cv2.COLOR_BGR2RGB)  # Convert in worker
                    self.latest_record_frame = record
                    if self.out_queue.qsize() < MAX_QUEUE_SIZE:
                        self.out_queue.put((self.cam_id, self.latest_preview_frame))
                    else:
            ...
            while not self.stop_event.is_set():
                ...
                ctx = self.get_overlay_ctx()
                preview = cv2.resize(frame, self.preview_size)  # No header for preview
                record = draw_header_to_size(frame, ctx, self.record_size)
                # Convert to RGB in worker to reduce GUI thread load
                preview_rgb = cv2.cvtColor(preview, cv2.COLOR_BGR2RGB)
                self.latest_preview_frame = preview_rgb
                self.latest_record_frame = record
                # Only send preview frame if enough time has elapsed (control FPS)
            ...
            logger.error(f"Camera {self.cam_id} error: {e}")
        finally:
...
class NSVApp(ctk.CTk):
    ...
    def get_overlay_context(self):
        ...
        return {
            ...
            'section_code': p.get('section_code','--'), 'direction': p.get('direction','--'),
            'lane': p.get('lane','--'), 'lat': lat, 'lon': lon, 'alt': alt, 'distance': dist,
            'chainage': chain, 'speed': f"{self.last_speed:.1f}", 'lrp': '--',
    ...
    def capture_images(self, chainage_label, lat=None, lon=None, alt=None):
        ...
        for cam_id, worker in self.cameras.items():
            ...
            if record_frame is None:
                ...
                record_frame = draw_header_to_size(cv2.cvtColor(preview, cv2.COLOR_RGB2BGR), ctx, RECORD_RES)
