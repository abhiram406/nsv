"""
NSV Controller v16 — dynamic, larger header (blue bar) + record-size media + labeled telemetry

Key features:
- Recording and saved images use RECORD_RES (e.g., 1280x720) with header rendered at that size.
- Preview uses PREVIEW_RES for UI, kept efficient.
- Blue header auto-sizes so all four rows fit on both preview and record frames (no clipping).
- Right panel shows explicit labels for Speed, Distance, Chainage, Latitude, Longitude, and Altitude.
- Sensor selection moved to Zero Calibration window, supports up to 13 sensors, 6 selected by default.
- Project setup fields pre-filled with specified values.
- Buttons arranged in two rows, with a speed progress bar.
"""
...
CAM_IDS = [0, 1, 2, 3]
CAM_NAMES = {0: "Back", 1: "Right", 2: "Front", 3: "Left"}
CAM_URLS = [
    "rtsp://192.168.1.101:554/stream0?username=admin&password=E10ADC3949BA59ABBE56E057F20F883E",  # Back
    "rtsp://192.168.1.28:554/stream1", # Right
    "rtsp://192.168.1.103:554/stream1", #Front
    "rtsp://192.168.1.102:554/stream1" # Left
]
...
GRID_COLOR  = "#263238"
SPINE_COLOR = "#607d8b"
HEADER_MAX_FRAC = 0.20       # header can grow up to 35% of frame height
HEADER_LINES = 5             # lines per column
LOGO_PATH = "cropped-Roadworks-LogoR-768x576-1.jpeg"       # Path to your logo image; replace as needed
SENSORS = 13
...
def draw_header_to_size(frame, ctx, size):
    """Compose header at target size (w,h) so all lines fit cleanly."""
    w, h = size
    ...
    # Font scale & spacing adapt to frame height
    if h >= 1080:
        font_scale = 0.6
    elif h >= 720:
        font_scale = 0.5
    ...
    # Measure text to compute line height robustly
    (_, th), base = cv2.getTextSize("Ag", FONT, font_scale, thickness)
    line_h = max(th + base, int(20 * font_scale))  # ensure spacing even on small frames
    pad_x = max(12, int(12 * (h / 480)))
    pad_top = max(8, int(10 * (h / 480)))
    pad_bottom = max(6, int(10 * (h / 480)))

    # Determine required header height for all lines
    ...
    # If needed content is taller than the cap, shrink font until it fits
    attempts = 0
    while needed_h > max_h and attempts < 5 and font_scale > 0.4:
        font_scale *= 0.9
        (_, th), base = cv2.getTextSize("Ag", FONT, font_scale, thickness)
        line_h = max(th + base, int(20 * font_scale))
        needed_h = pad_top + HEADER_LINES * line_h + pad_bottom
        attempts += 1
        ...
        try:
            if not self._open_cap():
                while not self.stop_event.is_set():
                    ...
                    preview = draw_header_to_size(make_testcard(self.preview_size[0], self.preview_size[1], "CAM NOT FOUND"), ctx, self.preview_size)
                    record  = draw_header_to_size(make_testcard(self.record_size[0], self.record_size[1], "CAM NOT FOUND"), ctx, self.record_size)
                    self.latest_preview_frame = preview
                    self.latest_record_frame  = record
                    self.out_queue.put((self.cam_id, preview))
            ...
            while not self.stop_event.is_set():
                ...
                if not ok or frame is None or frame.size == 0:
                    frame = make_testcard(self.record_size[0], self.record_size[1], f"{CAM_NAMES.get(self.cam_id, f'CAM {self.cam_id}')} ERROR")
                ctx = self.get_overlay_ctx()
...
class SensorWorker(threading.Thread):
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            vals = [abs(raw[i] - offs_val) for i in range(SENSORS)]
...
class NSVApp(ctk.CTk):
    ...
    def build_right_panel(self, parent):
        ...
        # Arrange buttons in two rows (3 in first row, 2 in second)
    ...
    def set_selection_state(self, enabled: bool):
        ...
        # Sensor checkboxes are in calibration window, no need to disable here
    ...
    def on_ten_m_tick(self, avgs, speed_kmph, latlonalt):
        ...
        self.window_buffer.append({"S": avgs, "speed": speed_kmph, "lat": lat, "lon": lon})
    ...
    def flush_measurement_window(self, final_ts, end_lat, end_lon, dir_sign, end_chain):
        ...
        avg_texture= np.nanmean(masked)  # use all selected
        ...
        lat_start = self.window_buffer[0]["lat"]
        lon_start = self.window_buffer[0]["lon"]
        row1 = [
        ...
        try:
            while True:
                ...
                if msg["type"] == "telemetry" and self.running:
                    ...
                    # Update progress bar (scale to 0-1, assuming max speed of 100 km/h)
    ...
    def open_calibrate(self):
        ...
        def apply_and_close():
            try:
                self.calib_value = float(e.get())
                #messagebox.showinfo("Calibration", f"Applied calibration: {self.calib_value}")
                win.destroy()
                self.rebuild_graphs()
            except Exception:
...
if __name__ == "__main__":
    ...
    splash.overrideredirect(True)  # Remove title bar for a clean popup
    splash.geometry("400x300")  # Small popup size; adjust as needed
    try:
        logo_img = ctk.CTkImage(light_image=Image.open(LOGO_PATH), size=(200, 200))  # Adjust size to fit your logo
    ...
    # Center the splash screen (optional)
    ...
    splash.after(6000, lambda: (splash.destroy(), app.deiconify()))
