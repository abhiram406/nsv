# NSV – Controller v6
# Brings back full v4 functionality + timestamped export folder
# What’s included:
# • Camera preview starts immediately; camera selection applies live
# • Start/Pause/Resume/Reset/Stop controls
# • Sensors: 5 TOF @ 100 Hz, bin/average every 10 m using DMI distance (simulated here)
# • Table columns: Bin end (m), Speed, Lat, Lon, Alt, TOF1..TOF5
# • Right sidebar metrics table: Rut, IRI, SMTD, RQI (Rut approx; others placeholders)
# • Export (and Stop→Export):
#     - Pauses data collection
#     - Prompts for base folder and creates stamped subfolder e.g., NSV_2025-08-19_11-42-30
#     - Saves Excel (if pandas+openpyxl) or CSV
#     - Saves .mp4 per selected camera into the same folder
#     - If run continues after export, new temp writers are re-opened
...
# -----------------------------
# Camera worker (preview always on; writes when writer present)
# -----------------------------
...
# -----------------------------
# Simulated hardware reads – replace with real integrations
# -----------------------------

def sim_speed_kmph(t):
    # Smooth speed profile 25–55 km/h
    return max(0.0, 40 + 12*math.sin(2*math.pi*t/20.0) + 3*random.uniform(-1, 1))


def sim_gps():
    # Hyderabad-ish jitter
...
def sim_tof_values():
    # Five beams in mm around 800
    return [800 + random.gauss(0, 5) for _ in range(5)]

# -----------------------------
# Sensor worker (100 Hz TOF; 10 m binning via DMI distance)
# -----------------------------
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        # video writers
        self.video_writers = {}    # cam_idx -> cv2.VideoWriter
        self.video_temp_paths = {} # cam_idx -> temp mp4 path
        ...
        # start camera previews immediately according to selection
    ...
    def build_left(self):
        ...
        for idx, cam in enumerate(CAM_INDEXES):
            ...
            chk = ctk.CTkCheckBox(cam_sel, text=f"Cam {cam}", variable=var,
                                   command=lambda cidx=cam, v=var: self._toggle_camera(cidx, v))
    ...
    def build_right(self):
        ...
        self.metrics = ttk.Treeview(metrics_frame, columns=("Left Wheel Path", "Right Wheel Path", "Average"), show="tree headings", height=4)
    ...
    def on_stop(self):
        if not self.running:
            return
        # export (which pauses), then teardown sensors + writers, reset; keep previews alive
        self.on_export()
        if self.sensor_worker:
    ...
    # -------- export with timestamped folder --------
    def on_export(self):
        # Pause during export
        self.paused_event.set()
        self.pause_btn.configure(text="Resume")
        ...
        try:
            ...
            messagebox.showerror("Export failed (data)", str(e))
            return

        # flush and copy videos for selected cameras
        copied_any = False
        try:
            # close writers to flush
            self._close_writers()
            for cam_idx, temp_path in list(self.video_temp_paths.items()):
                if temp_path and os.path.exists(temp_path):
                    out_video = os.path.join(out_dir, f"{os.path.basename(stamp)}_cam{cam_idx}.mp4")
                    try:
            ...
            print("Writer close/copy error:", e)

        # if continuing to run after export (user might hit Resume), open fresh writers
        if self.running:
            self._open_writers()
