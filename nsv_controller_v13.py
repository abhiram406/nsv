        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if not ok or frame is None or frame.size == 0:
                    frame = make_testcard(self.record_size[0], self.record_size[1], f"{CAM_NAMES.get(self.cam_index, f'CAM {self.cam_index}')} ERROR")
                ctx = self.get_overlay_ctx()
...
class NSVApp(ctk.CTk):
    ...
    def capture_images(self, chainage_label, lat=None, lon=None, alt=None):
        ...
        for cam_idx, worker in self.cameras.items():
            if not self.cam_selected.get(cam_idx, tk.BooleanVar(value=True)).get(): continue
