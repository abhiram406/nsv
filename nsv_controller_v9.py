# NSV Controller v9 — COMPLETE
# -------------------------------------------------------------
# New in v9 (per your list):
# 1) Calibration window stays in foreground (modal: grab_set, transient, topmost) until closed.
# 2) Single calibration value for all 6 sensors.
# 3) "Apply and Close" button applies calibration and closes the popup.
# 4) Export shows a confirmation popup when files are written.
# 5) Sensor values S1..S6 forced to absolute (non‑negative) after calibration.
# 6) Dark theme UI + dark Matplotlib; plot lines are fluorescent green.
# 7) Camera labels & filenames: Front, Left, Right, Back. Streams/images saved with these names.
# 8) Selection checkboxes for cameras & sensors; selections locked during run and unlocked on Stop/Reset.
# 9) Images folder contains subfolders Front/Left/Right/Back; frames saved into respective subfolders.
# -------------------------------------------------------------
...
class SensorWorker(threading.Thread):
    def __init__(self, ui_queue, paused_event, calib_getter, ten_m_callback, sensor_selected_getter):
        super().__init__(daemon=True)
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            # Respect sensor selection: still buffer for charts, but callback will mask for export/calcs
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("NSV Controller v9 — COMPLETE")
    ...
    def build_layout(self):
        ...
        self.build_selections(self.center)  # camera & sensor selection module below cameras
    ...
    # ---------- Selections (cameras & sensors) ----------
    def build_selections(self, parent):
        sel = ctk.CTkFrame(parent)
        sel.grid(row=1, column=0, columnspan=2, sticky="ew", padx=8, pady=(0,4))
        sel.grid_columnconfigure(0, weight=1)
        sel.grid_columnconfigure(1, weight=1)

        cams = ctk.CTkFrame(sel)
        cams.grid(row=0, column=0, sticky="w", padx=6)
        ctk.CTkLabel(cams, text="Export Cameras:").grid(row=0, column=0, columnspan=4, sticky="w")
        for i, cam_idx in enumerate(CAM_INDEXES):
            name = CAM_NAMES[cam_idx]
            chk = ctk.CTkCheckBox(cams, text=name, variable=self.cam_selected[cam_idx])
            chk.grid(row=1, column=i, padx=4, pady=2, sticky="w")
        self.cam_checkboxes = [w for w in cams.winfo_children() if isinstance(w, ctk.CTkCheckBox)]

        sens = ctk.CTkFrame(sel)
        sens.grid(row=0, column=1, sticky="w", padx=6)
        ctk.CTkLabel(sens, text="Export Sensors:").grid(row=0, column=0, columnspan=6, sticky="w")
        self.sensor_checkboxes = []
        for i in range(6):
            chk = ctk.CTkCheckBox(sens, text=f"S{i+1}", variable=self.sensor_selected[i])
            chk.grid(row=1, column=i, padx=2, pady=2, sticky="w")
    ...
    def build_graphs(self, parent):
        ...
        self.graph_frame.grid(row=2, column=0, columnspan=2, sticky="nsew", padx=8, pady=(4,8))
        self.graph_frame.grid_rowconfigure(0, weight=1)
        self.graph_frame.grid_columnconfigure(0, weight=1)
        # Dark figure to match UI
        fig = Figure(figsize=(9,4.2), dpi=100, facecolor="#0b0f14")
        self.axes = []
        for i in range(6):
            ax = fig.add_subplot(2,3,i+1, facecolor="#0b0f14")
            ax.set_title(f"S{i+1}", fontsize=9, color="white")
            ax.set_xlabel("Seg", fontsize=8, color="white")
    ...
    def set_project_specs(self):
        ...
        # Lock left panel; enable right controls and selections
        self.set_left_state(False)
        self.set_controls_state(True)
        self.set_selection_state(True)  # allow choosing cameras/sensors BEFORE starting
    ...
    def flush_measurement_window(self, final_ts, end_lat, end_lon, dir_sign, end_chain):
        if not self.window_buffer:
            return
        # Average S across buffered segments, then apply selection mask for calculations
        S_matrix = np.array([w["S"] for w in self.window_buffer])  # shape (n,6)
        mean_S = np.nanmean(S_matrix, axis=0)  # (6,)
        sel = np.array(self.get_sensor_selection(), dtype=bool)
        # Build a copy that masks unselected sensors as NaN for calc
    ...
    def capture_images(self, chainage_label):
        ...
        for cam_idx, worker in self.cameras.items():
            ...
            # Save into camera-specific subfolder
    ...
    def open_calibrate(self):
        ...
        win.title("Zero Calibration — All Sensors")
        ...
        try:
            ...
            pass
        ctk.CTkLabel(win, text="Calibration value (applied to all sensors)").pack(pady=(16,4))
        e = ctk.CTkEntry(win)
