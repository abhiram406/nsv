"""
NSV Controller v17 — reduced header height and font, no header in preview, maximized GUI

Key features:
- Recording and saved images use RECORD_RES (e.g., 1280x720) with a smaller header (max 20% frame height).
- Preview uses PREVIEW_RES without header or text for clean camera view.
...
- Sensor selection supports up to 13 sensors, 6 selected by default.
...
GRID_COLOR  = "#263238"
SPINE_COLOR = "#607d8b"
HEADER_MAX_FRAC = 0.20       # Reduced header height to 20% of frame
HEADER_LINES = 4             # Lines per column
LOGO_PATH = "logo.png"       # Path to your logo image; replace as needed
SENSORS = 13
...
def draw_header_to_size(frame, ctx, size):
    """Compose header at target size (w,h) with smaller font for recorded video/images."""
    w, h = size
    ...
    # Reduced font scale for smaller header
    if h >= 1080:
        font_scale = 0.7  # Reduced from 0.9
    elif h >= 720:
        font_scale = 0.6  # Reduced from 0.75
    elif h >= 480:
        font_scale = 0.5  # Reduced from 0.62
    else:
        font_scale = 0.4  # Reduced from 0.52
    thickness = 1

    # Measure text to compute line height
    (_, th), base = cv2.getTextSize("Ag", FONT, font_scale, thickness)
    line_h = max(th + base, int(16 * font_scale))  # Reduced spacing
...
if __name__ == "__main__":
    ...
    try:
        logo_img = ctk.CTkImage(light_image=Image.open(LOGO_PATH), size=(300, 200))  # Adjust size to fit your logo
