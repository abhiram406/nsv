...
EXPECTED_BLOCK_SIZE = 100  # Fetch one value per sensor per cycle for 100 Hz
TOF_HZ = 100
...
# Show only the last N points in laser plots
PLOT_MAX_POINTS = 10  # NEW


# GPS Configuration
GPS_COM_PORT = "COM30"
GPS_BAUDRATE = 57600  # Emlid Reach M2 default, can also be 9600, 38400, 57600, 230400
...
class GPSReader:
    ...
    Parses GGA and RMC sentences for position, altitude, and speed.
    """
    def __init__(self, port=GPS_COM_PORT, baudrate=GPS_BAUDRATE):
        ...
        self.fix_quality = 0  # 0=invalid, 1=GPS fix, 2=DGPS fix, 4=RTK fixed, 5=RTK float
    ...
    def parse_nmea_coordinate(self, coord_str, direction):
        ...
        Returns:
            Decimal degrees (float) or None if invalid
        """
        try:
    ...
    def parse_rmc(self, sentence):
        ...
        try:
            ...
            with self.lock:
                ...
                self.speed_kmph = self.speed_knots * 1.852  # Convert knots to km/h
                self.last_update = time.time()
    ...
    def parse_vtg(self, sentence):
        ...
        Format: $GPVTG,course_true,T,course_mag,M,speed_knots,N,speed_kmh,K,...
        """
        try:
            parts = sentence.split(',')
            if len(parts) < 9:
    ...
    def get_position(self):
        ...
        Returns:
            tuple: (latitude, longitude, altitude, speed_kmph) or (None, None, None, 0.0) if no fix
        """
        with self.lock:
...
class DMISimulator:
    ...
    Provides speed and distance measurements independent of GPS.
    This will be replaced with real DMI hardware in the future.
    """
    def __init__(self, base_speed_kmh=30.0, speed_variation=5.0):
        ...
        # Simulation parameters for realistic speed variation
        self.speed_change_interval = 2.0  # Change speed every 2 seconds
...
def read_laser_values(sensors):
    ...
    for i, sensor in enumerate(sensors):
        if sensor is None:
            # Sensor not initialized - just return NaN silently
            continue
        try:
            ...
            if sensor.DataAvail() >= EXPECTED_BLOCK_SIZE and sensor.GetLastError() == ERR_CODE.ERR_NOERROR:
                ...
                # Suppress data error warnings - just return NaN
            # Suppress data unavailable warnings - just return NaN
        except Exception as e:
            # Only log actual exceptions, not missing data
...
def initialize_lasers_with_timeout():
    ...
    initialized_count = 0
    timeout_sec = 3.0  # Max wait per port

    # Your actual ports (only 6 connected)
    ...
    def try_init(port, idx, result_queue):
        ...
        try:
            ...
            # Critical: Set serial timeout
            sensor.SetParameterInt("IP_Timeout", int(timeout_sec * 1000))  # ms
    ...
    def _try_open(self, backend_flag):
        ...
        try:
            ...
            if backend_flag == cv2.CAP_FFMPEG:
                os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = "rtsp_transport;udp|fflags;nobuffer|probesize;32768|analyzeduration;100000|timeout;2000000"
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            if self.cap is None or not self.is_connected:
                # Show test card while disconnected
...
class SensorWorker(threading.Thread):
    def __init__(self, ui_queue, paused_event, calib_getter, ten_m_callback,
                 ...
                 sensors=None):  # NEW: accept pre-initialized sensors
        super().__init__(daemon=True)
        ...
        # Use provided sensors if available; otherwise, initialize here (fallback)
        if sensors is not None:
            self.sensors = sensors
            self._owns_sensors = False   # NEW
        else:
            self.sensors = initialize_lasers_with_timeout()
            self._owns_sensors = True    # NEW
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            # Get GPS position (lat/lon/alt only - no speed from GPS)
            lat, lon, alt, _ = self.gps_reader.get_position()
            
            # Get speed and distance from DMI simulator
            current_speed_kmh = self.dmi_simulator.get_speed_kmh()
            current_distance_m = self.dmi_simulator.get_distance_m()
            
            # If no GPS fix, log warning but continue with DMI data
            ...
            # Send telemetry update with DMI speed and distance
            self.ui_queue.put({
                "type": "telemetry",
            ...
            # Check if we've crossed a 10-meter boundary
    ...
    def stop(self):
        self.stop_event.set()
        if getattr(self, "_owns_sensors", False):  # NEW
            cleanup_lasers(self.sensors)
        # If not owned, NSVApp will clean up on exit
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        if self.gps_reader.connect():
            ...
            logger.error("Failed to connect to GPS - position data will not be available")
            messagebox.showwarning("GPS Warning", 
                               f"Failed to connect to GPS on {GPS_COM_PORT}.\n"
                               f"Position data (Lat/Lon) will not be available.\n"
                               f"Speed and distance will use simulated DMI.")
        
        # Initialize DMI Simulator (for speed and distance)
        ...
        logger.info("DMI simulator started (simulating 25-35 km/h)")
                # ---- Initialize Lasers at startup (alongside GPS & Cameras) ----  # NEW
        ...
        try:
            ...
            logger.error(f"Laser init at startup failed: {e}")
            self.sensors = [None] * SENSORS
            self.laser_status_light.configure(text_color="#d32f2f")
            self.laser_status_text.configure(text="Init error")
    ...
    def update_laser_status(self):
        ...
        if self.sensor_worker and hasattr(self.sensor_worker, 'sensors'):
            connected_count = sum(1 for s in self.sensor_worker.sensors if s is not None)
            ...
            if valid_data_count > 0:
                ...
                if valid_data_count <= 5:
                    # Few sensors - show specific sensor numbers
    ...
    def set_project_specs(self):
        ...
        # Start device status monitoring and GPS updates
    ...
    def on_start(self):
        ...
        # Check if GPS has a fix (warning only, not blocking)
        lat, lon, alt, speed = self.gps_reader.get_position()
        if lat is None or lon is None:
            fix_info = self.gps_reader.get_fix_info()
            logger.warning(f"GPS Fix Status: {fix_info['fix_type']}, Satellites: {fix_info['satellites']}")
            # Don't block - DMI will provide speed/distance, GPS position not critical
        
        self.running = True
        self.paused_event.clear()
        self.set_status_color("yellow")
        self.lbl_status.configure(text="Status: calibrating...")
        self.set_selection_state(False)
        
        # Start sensor worker if not already running
        if self.sensor_worker is None:
            # Reset DMI distance counter for this run
            ...
            self.sensor_worker = SensorWorker(
                ...
                self.gps_reader, self.dmi_simulator, sensors=self.sensors  # NEW
            )
            self.sensor_worker.start()

        
        # Wait for valid sensor readings before calibrating
        def wait_and_calibrate():
            max_wait_cycles = 50  # Wait up to ~5 seconds (50 * 100ms)
            wait_count = 0
            
            def check_readings():
                nonlocal wait_count
                # Check if we have ANY valid readings from sensors (at least 1 sensor)
                valid_readings = sum(1 for val in self.latest_raw if np.isfinite(val))
                
                if valid_readings >= 1:  # At least one sensor has valid data
                    # Capture calibration offsets independently for each sensor
                    # Each sensor gets its own offset if it has valid data, otherwise 0.0
                    self.calib_values = []
                    calibrated_sensors = []
                    for i in range(SENSORS):
                        if np.isfinite(self.latest_raw[i]):
                            self.calib_values.append(self.latest_raw[i])
                            calibrated_sensors.append(i + 1)
                        else:
                            self.calib_values.append(0.0)
                    
                    logger.info(f"Calibration captured for sensors: {calibrated_sensors}")
                    logger.info(f"Calibration values: {self.calib_values}")
                    
                    # Now fully start the system
                    self.set_status_color("green")
                    self.lbl_status.configure(text="Status: running")
                    if not self.video_writers: 
                        self.open_writers()
                    
                    # Create detailed message about which sensors were calibrated
                    sensor_status = ", ".join([f"S{i+1}" for i in range(SENSORS) if np.isfinite(self.latest_raw[i])])
                    uncalibrated = ", ".join([f"S{i+1}" for i in range(SENSORS) if not np.isfinite(self.latest_raw[i])])
                    
                    msg = f"✓ Calibrated sensors ({valid_readings}/{SENSORS}):\n{sensor_status}\n\n"
                    if uncalibrated:
                        msg += f"⚠ Uncalibrated sensors (will use zero offset):\n{uncalibrated}"
                    
                    messagebox.showinfo("Calibration Complete", msg)
                    
                elif wait_count < max_wait_cycles:
                    wait_count += 1
                    # Keep waiting
                    self.after(100, check_readings)
                else:
                    # Timeout - use zeros for all sensors
                    logger.warning("Calibration timeout - no valid sensor readings found")
                    self.calib_values = [0.0] * SENSORS
                    self.set_status_color("green")
                    self.lbl_status.configure(text="Status: running")
                    if not self.video_writers: 
                        self.open_writers()
                    messagebox.showwarning("Calibration Timeout", 
                                         "No valid sensor readings detected after 5 seconds.\n\n"
                                         "All sensors will use zero calibration offsets.\n"
                                         "Check sensor connections and COM ports.")
            
            check_readings()
        
        # Start calibration check after a brief delay
        self.after(200, wait_and_calibrate)
    ...
    def on_export(self):
        ...
        t1_cols = [
            ...
            "Left IRI (mm)","Right IRI (mm)","Average IRI (mm)",
        ...
        rut_cols = [c for c in t1_cols if c not in ("Left IRI (mm)","Right IRI (mm)","Average IRI (mm)","Average Texture (mm)")]
        iri_cols = [c for c in t1_cols if c not in ("Left Rut (mm)","Right Rut (mm)","Average Rut (mm)","Average Texture (mm)")]
        tex_cols = [c for c in t1_cols if c not in ("Left IRI (mm)","Right IRI (mm)","Average IRI (mm)","Left Rut (mm)","Right Rut (mm)","Average Rut (mm)")]
        try:
            os.makedirs(self.data_dir, exist_ok=True)
        ...
        def update_sensor_readings():
            ...
            for i in range(SENSORS):
                ...
                if np.isfinite(raw_val):
                    ...
                    # Show both raw and calibrated values
    ...
    def on_close(self):
        try:
            ...
            # Clean up shared lasers here (since app owns them)  # NEW
            try:
