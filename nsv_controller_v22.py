...
def initialize_lasers():
    ...
    for i in range(SENSORS):
        try:
            ...
            if sensor.iSensor == 0:
                # Silently skip - sensor creation failed
                continue
            ...
            sensor.SetParameterInt("IP_EnableLogging", 0)  # Disable individual sensor logging
            # Open sensor
            sensor.OpenSensor()
            if sensor.GetLastError() != ERR_CODE.ERR_NOERROR:
                # Silently skip - sensor failed to open
                sensor.CloseSensor()
            ...
            # Silently skip initialization errors
            logger.debug(f"Sensor {i+1} setup failed: {str(e)}")
    
    # Log summary instead of individual failures
    logger.info(f"Laser initialization complete: {initialized_count}/{SENSORS} sensors connected")
    return sensors
...
class SensorWorker(threading.Thread):
    def __init__(self, ui_queue, paused_event, calib_getter, ten_m_callback, sensor_selected_getter, raw_setter, gps_reader):
        super().__init__(daemon=True)
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            # Get GPS data from Emlid Reach M2
            lat, lon, alt, speed_gps = self.gps_reader.get_position()
            
            # If no GPS fix, log warning and use last known position
            ...
            if self.prev_lat is not None and self.prev_lon is not None:
                ...
                dt = t - self.prev_t if self.prev_t else 0
                # Use GPS speed if available and reasonable, otherwise calculate from distance
                if speed_gps > 0.1:
                    self.last_speed = speed_gps
                elif dt > 0:
                ...
                self.last_speed = speed_gps if speed_gps is not None else 0.0
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        if self.gps_reader.connect():
            ...
            logger.error("Failed to connect to GPS - system will not function properly")
            messagebox.showerror("GPS Error", 
                               f"Failed to connect to GPS on {GPS_COM_PORT}.\n"
                               f"Please check:\n"
                               f"- GPS is connected to {GPS_COM_PORT}\n"
                               f"- Correct COM port in configuration\n"
                               f"- pyserial is installed: pip install pyserial")
    ...
    def on_start(self):
        ...
        # Check if GPS has a fix before starting
        lat, lon, alt, speed = self.gps_reader.get_position()
        if lat is None or lon is None:
            fix_info = self.gps_reader.get_fix_info()
            response = messagebox.askyesno("No GPS Fix", 
                                          f"GPS Fix Status: {fix_info['fix_type']}\n"
                                          f"Satellites: {fix_info['satellites']}\n\n"
                                          f"No GPS fix available. Starting without GPS may result in invalid data.\n\n"
                                          f"Continue anyway?")
            if not response:
                return
    ...
    def open_sensor_selection(self):
        ...
        sens_frame = ctk.CTkScrollableFrame(win, height=300)
        ...
        for i in range(SENSORS):
            ...
            chk = ctk.CTkCheckBox(sensor_frame, text=f"S{i+1}", variable=self.sensor_selected[i])
        ...
        def update_sensor_readings():
            ...
            self.sensor_status_label.configure(
                text=f"Live Readings: {valid_count}/{SENSORS} sensors active"
            )
    ...
    def on_close(self):
        try:
            if self.sensor_worker:
                ...
                self.sensor_worker = None
            if self.gps_worker:
                try: self.gps_worker.stop(); self.gps_worker.join(timeout=1.0)
                except Exception: pass
                self.gps_worker = None
            if self.gps_reader:
