...
class NSVApp(ctk.CTk):
    ...
    def on_reset(self):
        ...
        self.calib_values = [0.0] * SENSORS
        logger.info("Calibration values reset to zero")
    ...
    def on_start(self):
        ...
        def wait_and_calibrate():
            ...
            def check_readings():
                nonlocal wait_count
                # Check if we have valid readings from sensors
                valid_readings = sum(1 for val in self.latest_raw if np.isfinite(val))
                
                if valid_readings >= SENSORS // 2:  # At least half the sensors have valid data
                    # Capture calibration offsets from current sensor readings
                    self.calib_values = [self.latest_raw[i] if np.isfinite(self.latest_raw[i]) else 0.0 for i in range(SENSORS)]
                    logger.info(f"Calibration captured: {self.calib_values}")
                    ...
                    messagebox.showinfo("Calibration Complete", 
                                      f"Sensors calibrated with {valid_readings}/{SENSORS} valid readings.\n"
                                      "Current sensor positions set as zero reference.")
                    ...
                    # Timeout - use zeros
                    logger.warning("Calibration timeout - using zero offsets")
                    self.calib_values = [0.0] * SENSORS
                    self.set_status_color("green")
                    self.lbl_status.configure(text="Status: running")
                    if not self.video_writers: 
                    ...
                    messagebox.showwarning("Calibration Warning", 
                                         "Unable to get valid sensor readings.\n"
                                         "Using zero calibration offsets.")
    ...
    def open_sensor_selection(self):
        ...
        ctk.CTkLabel(win, text="Select Sensors to Export", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=(10,20))

        # Create a frame for checkboxes
        sens_frame = ctk.CTkFrame(win)
        sens_frame.pack(fill="both", expand=True, padx=20, pady=10)

        # Create sensor checkboxes in a grid (3 columns)
        self.sensor_checkboxes = []
        for i in range(SENSORS):
            row = i // 3
            col = i % 3
            chk = ctk.CTkCheckBox(sens_frame, text=f"Sensor {i+1}", variable=self.sensor_selected[i])
        ...
        def apply_and_close():
            self.rebuild_graphs()
            messagebox.showinfo("Sensors Updated", f"{sum(v.get() for v in self.sensor_selected)} sensors selected for export and plotting.")
            win.destroy()
