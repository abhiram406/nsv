# NSV Controller v7 — Fix 3
# Changes requested:
# 1) "Start Project" → "Set Project Specifications". It VALIDATES + PREPARES folders, but DOES NOT start sensors.
#    Sensors/graphs only run via right-panel controls (Start/Pause/Stop/Export).
# 2) Until specs are validated, center & right functionality inactive (buttons disabled, graphs empty).
# 3) After specs validated, LEFT panel becomes inactive (locked) until user clicks new "Reset" (on right panel).
# 4) Lane dropdown values auto-switch based on Direction (Increasing → L1..L10, Decreasing → R1..R10).
#    (Already implemented, retained here.)
# Also keeps prior fixes: layout sizing, folder structure under Run_*, partial-window flush on export, etc.
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("NSV Controller v7 — Fix 3")
        ...
        self.controls_enabled = False  # central/right functionality gate
    ...
    def build_project_setup(self, f):
        ...
        self.cmb_dir = ctk.CTkComboBox(f, values=["Increasing", "Decreasing"], width=entry_w)
        self.cmb_dir.set("Increasing")
        self.cmb_dir.bind("<<ComboboxSelected>>", self.on_direction_change)
        add_row(6, "Direction", self.cmb_dir)

        self.cmb_lane = ctk.CTkComboBox(f, values=[f"L{i}" for i in range(1,11)], width=entry_w)
        self.cmb_lane.set("L1")
        add_row(7, "Lane No.", self.cmb_lane)
        ...
        self.left_widgets = [
            ...
            self.cmb_dir, self.cmb_lane, self.cmb_pave, self.ent_chainage, self.ent_section_km, self.btn_set_specs
        ]

    def on_direction_change(self, *_):
        dirv = self.cmb_dir.get()
        if dirv == "Increasing":
            self.cmb_lane.configure(values=[f"L{i}" for i in range(1,11)])
            if not self.cmb_lane.get().startswith("L"):
                self.cmb_lane.set("L1")
        else:
            self.cmb_lane.configure(values=[f"R{i}" for i in range(1,11)])
            if not self.cmb_lane.get().startswith("R"):
                self.cmb_lane.set("R1")
    ...
    def set_project_specs(self):
        ...
        direction = self.cmb_dir.get().strip()
        lane = self.cmb_lane.get().strip()
