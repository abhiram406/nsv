# NSV Controller v10 — COMPLETE
# -------------------------------------------------------------
# New in v10 (on top of v9):
# 1) Calibration window shows LIVE raw S1..S6 values (pre-calibration) and stays modal in foreground.
# 2) Single calibration value for all sensors retained.
# 3) "Apply and Close" button closes the calibration popup after applying.
# 4) Export already confirms with a popup (kept).
# 5) Sensor values used for processing are absolute (non-negative).
# 6) Dark theme + neon-green graphs (kept) and x-axis values removed (only y-axis shown).
# 7) Camera labels kept (Front/Left/Right/Back). Overlay: each frame (video+images) now has a BLUE header bar
#    containing project, GPS, speed, chainage, date/time (3-column layout) in white text.
# 8) Camera & sensor selection checkboxes moved to LEFT panel (below inputs). Selections lock while running and
#    unlock on Stop/Reset. "Set Project Specifications" button moved to the bottom of the left panel.
# 9) Reset also restores all right-panel labels to initial states.
# -------------------------------------------------------------
...
CAM_RES = (360, 202)     # compact preview size for 2×2 grid (16:9-ish incl. header)
CAM_FPS = 25
TOF_HZ = 100
TEN_M_METERS = 10.0
TEN_M_KM = 0.01
NEON_GREEN = "#39ff14"
HEADER_H = 64            # pixels for the blue header overlay
HEADER_COLOR = (200, 80, 0)  # BGR-ish blue (adjusted for good contrast); tweak as needed
...
def make_testcard(w, h, text="NO CAMERA"):
    ...
    for i, c in enumerate(bars):
        img[:, i*bw:(i+1)*bw] = c
    cv2.putText(img, text, (10, h//2), FONT, 0.7, (0,0,0), 3, cv2.LINE_AA)
    cv2.putText(img, text, (10, h//2), FONT, 0.7, (255,255,255), 1, cv2.LINE_AA)
    return img
...
class CameraWorker(threading.Thread):
    def __init__(self, cam_index, out_queue, get_writer, get_overlay_ctx, width=CAM_RES[0], height=CAM_RES[1]):
    ...
    def draw_header(self, frame, ctx):
        # Ensure size
        ...
        col1 = [
            ...
            f"Old NH Number: {ctx.get('oldnh','--')}",
            f"Direction: {ctx.get('direction','--')}",
        ]
        col2 = [
            f"Lane no.: {ctx.get('lane','--')}",
            f"Latitude: {ctx.get('lat','--')}",
            f"Longitude: {ctx.get('lon','--')}",
            f"Altitude: {ctx.get('alt','--')} m",
        ]
        col3 = [
            f"Distance: {ctx.get('distance','--')} m",
            f"Chainage: {ctx.get('chainage','--')}",
            f"Speed: {ctx.get('speed','--')} km/h",
            f"{ctx.get('date','--')}  {ctx.get('time','--')}",
        ]
        cols = [col1, col2, col3]
        # positions
        pad_x = 12; pad_y = 18; line_h = 16
        col_w = frame.shape[1] // 3
        for ci, lines in enumerate(cols):
            x = pad_x + ci*col_w
            y = 18
            for li, text in enumerate(lines):
                # vertically clip into header
                yy = y + li*line_h
                if yy < header_h - 3:
                    cv2.putText(frame, text, (x, yy), FONT, 0.45, TEXT_COLOR, 1, cv2.LINE_AA)
        return frame
        ...
        try:
            ...
            if not (self.cap and self.cap.isOpened()):
                while not self.stop_event.is_set():
                    frame = make_testcard(self.width, self.height, f"{CAM_NAMES.get(self.cam_index, f'CAM {self.cam_index}')} NOT FOUND")
                    ctx = self.get_overlay_ctx()
                    frame = self.draw_header(frame, ctx)
            ...
            while not self.stop_event.is_set():
                ...
                if not ok:
                    frame = make_testcard(self.width, self.height, f"{CAM_NAMES.get(self.cam_index, f'CAM {self.cam_index}')} ERROR")
                else:
                    frame = cv2.resize(frame, (self.width, self.height))
                # Overlay header based on current context
                ctx = self.get_overlay_ctx()
                frame = self.draw_header(frame, ctx)
                self.latest_frame = frame
                self.out_queue.put((self.cam_index, frame))
...
class SensorWorker(threading.Thread):
    def __init__(self, ui_queue, paused_event, calib_getter, ten_m_callback, sensor_selected_getter, raw_setter):
        ...
        self.raw_setter = raw_setter  # callback to set latest RAW values (pre-calibration) for calibration window
        self.stop_event = threading.Event()
        self.reset()
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            lat, lon, alt = sim_gps()
            raw = sim_tof_values()
            # Expose RAW for calibration window
            try:
                self.raw_setter(raw)
            except Exception:
                pass
            # Apply single offset to all sensors, then absolute value
            offs_val = self.calib_getter()  # returns a single float
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("NSV Controller v10 — COMPLETE")
        self.geometry(WINDOW_GEOM)

        # Grid: lock side panel min widths
        ...
        self.project = None
        self.calib_value = 0.0  # single calibration value for all sensors
        ...
        # Latest telemetry for overlay
        self.last_lat = None
        self.last_lon = None
        self.last_alt = None
        self.last_speed = 0.0
        self.last_distance_m = 0.0
        self.latest_raw = [float('nan')]*6  # for calibration popup (raw)
    ...
    # ---------- Left Panel: Project Setup + Selections ----------
    def build_left_panel(self, f):
        ...
        # Selections (moved here from center)
        row0 = 11
        ...
        # Set Project Specifications at the bottom of the left panel
        ...
        # track left widgets to disable after validation (inputs only; keep selection lock managed separately)
        self.left_widgets = [
    ...
    # ---------- Centre: Cameras (2×2 compact previews with overlay) ----------
    def build_cameras(self, parent):
        ...
        for i, cam_idx in enumerate(CAM_INDEXES):
            r, c = divmod(i, 2)
            name = CAM_NAMES.get(cam_idx, f"Camera {cam_idx}")
            frame = ctk.CTkFrame(self.cam_frame)
            frame.grid(row=r, column=c, padx=6, pady=6, sticky="n")
            lbl_title = ctk.CTkLabel(frame, text=name)
            lbl_title.pack(anchor="w")
            lbl = ctk.CTkLabel(frame, text=name, width=CAM_RES[0], height=CAM_RES[1])
            lbl.pack()
            self.cam_labels.append(lbl)

    # ---------- Centre: Graphs (2×3 compact; x-axis removed) ----------
    def build_graphs(self, parent):
        ...
        fig = Figure(figsize=(9.6,4.6), dpi=100, facecolor="#0b0f14")
        self.axes = []
        for i in range(6):
            ax = fig.add_subplot(2,3,i+1, facecolor="#0b0f14")
    ...
    def set_selection_state(self, enabled: bool):
        state = "normal" if enabled else "disabled"
        for cb in getattr(self, 'cam_checkboxes', []):
            cb.configure(state=state)
        for cb in getattr(self, 'sensor_checkboxes', []):
            cb.configure(state=state)
    ...
    def get_writer_for(self, cam_idx):
        if not self.running or self.paused_event.is_set() or self.stream_dir is None:
            return None
        # Only write streams for selected cameras
        if not self.cam_selected[cam_idx].get():
            return None
        return self.video_writers.get(cam_idx)

    def get_overlay_context(self):
        # Prepare the context for header overlay
        survey = self.project['survey'] if self.project else '--'
        nh = self.project['nh'] if self.project else '--'
        oldnh = self.project['oldnh'] if self.project else '--'
        direction = self.project['direction'] if self.project else '--'
        lane = self.project['lane'] if self.project else '--'
    ...
    def set_project_specs(self):
        ...
        # Lock left inputs; enable right controls and selections
        self.set_left_state(False)
        self.set_controls_state(True)
        # Lock selections after specs are set
    ...
    def build_project_paths(self, base, survey, direction, lane):
        ...
        # Create camera subfolders inside Images
        os.makedirs(self.images_dir, exist_ok=True)
        for name in CAM_NAMES.values():
    ...
    def on_reset(self):
        ...
        self.current_chainage_km = None
        # Reset right-side labels to initial
        self.set_status_color("red")
        ...
        self.var_lon.set("Lon: --")
        self.var_alt.set("Alt: -- m")
        # Clear graphs
        self.graph_data = [[] for _ in range(6)]
        for line in self.lines:
            line.set_data([], [])
    ...
    def on_start(self):
        ...
        if self.sensor_worker is None:
            self.sensor_worker = SensorWorker(
                self.ui_queue,
                self.paused_event,
                self.get_calibration_value,
                self.on_ten_m_tick,
                self.get_sensor_selection,
                self.set_latest_raw
            )
    ...
    def on_export(self):
        ...
        try:
            ...
            # confirmation popup per request
    ...
    def on_ten_m_tick(self, avg_six, speed_kmph, latlonalt):
        ...
        self.last_speed = speed_kmph
        self.last_distance_m += TEN_M_METERS
        # Table 2 (10 m)
    ...
    def flush_measurement_window(self, final_ts, end_lat, end_lon, dir_sign, end_chain):
        if not self.window_buffer:
            return
        S_matrix = np.array([w["S"] for w in self.window_buffer])  # shape (n,6)
        ...
        row1 = [final_ts, self.project["nh"], f"{start_c:.3f}", f"{end_c:.3f}", self.project["direction"], self.project["lane"], f"{speed_mean:.1f}", f"{left_iri if not np.isnan(left_iri) else 0:.3f}", f"{right_iri if not np.isnan(right_iri) else 0:.3f}", f"{avg_iri if not np.isnan(avg_iri) else 0:.3f}", f"{left_rut if not np.isnan(left_rut) else 0:.1f}", f"{right_rut if not np.isnan(right_rut) else 0:.1f}", f"{avg_rut if not np.isnan(avg_rut) else 0:.1f}", f"{avg_texture if not np.isnan(avg_texture) else 0:.3f}", f"{lat_start:.6f}", f"{lon_start:.6f}", f"{end_lat:.6f}", f"{end_lon:.6f}"]
        self.table1_rows.append(row1)
        self.measure_accum_km = 0.0
        self.window_buffer = []

    def set_latest_raw(self, raw_vals):
        # raw_vals is a list of 6 floats
    ...
    def capture_images(self, chainage_label, lat=None, lon=None, alt=None):
        ...
        for cam_idx, worker in self.cameras.items():
            ...
            try:
                # Ensure header already present on latest_frame; if not, draw again with current ctx
                out = worker.draw_header(frame.copy(), ctx)
    ...
    def open_writers(self):
        ...
        for cam_idx in CAM_INDEXES:
            if not self.cam_selected[cam_idx].get():  # only open for selected cameras
                continue
            name = CAM_NAMES.get(cam_idx, f"CAM{cam_idx}")
    ...
    def open_calibrate(self):
        ...
        win.title("Zero Calibration — All Sensors (Live Raw)")
        win.geometry("420x300")
        # keep popup in foreground (modal)
        win.transient(self)
        win.grab_set()
        try:
        ...
        e = ctk.CTkEntry(win)
        e.insert(0, str(self.calib_value))
        e.pack(pady=4)
        # Live RAW table
        table = ctk.CTkFrame(win)
        table.pack(fill="x", padx=10, pady=8)
        title = ctk.CTkLabel(table, text="Live RAW sensor values (S1..S6)")
        title.grid(row=0, column=0, columnspan=6, pady=(0,6))
        self.raw_labels = []
        for i in range(6):
            ctk.CTkLabel(table, text=f"S{i+1}").grid(row=1, column=i, padx=6)
            lbl = ctk.CTkLabel(table, text="--")
            lbl.grid(row=2, column=i, padx=6)
            self.raw_labels.append(lbl)

        def tick():
            # Update labels from self.latest_raw
            for i in range(6):
                try:
                    v = self.latest_raw[i]
                    self.raw_labels[i].configure(text=f"{v:.2f}")
                except Exception:
                    self.raw_labels[i].configure(text="--")
            if win.winfo_exists():
                win.after(100, tick)
        tick()
        ...
        def apply_and_close():
            try:
                val = float(e.get())
                self.calib_value = val
                messagebox.showinfo("Calibration", f"Applied calibration: {val}")
                win.destroy()
            except Exception:
