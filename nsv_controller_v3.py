# NSV – Controller v3
# Additions vs v2:
# 1) Reset button clears table and internal bin state
# 2) Pause/Resume toggle button
# 3) Stop now exports data + videos, then resets and stops all threads
# 4) Camera selection checkboxes (choose which cameras to use)
# 5) Export videos (.mp4) for the selected cameras to same folder as data export
# 6) Table now includes Speed (km/h), Latitude, Longitude, Altitude along with TOF averages
...
# -----------------------------
# Camera worker with pause + recording
# -----------------------------
class CameraWorker(threading.Thread):
    def __init__(self, cam_index, out_queue, paused_event, get_writer, width=CAM_RES[0], height=CAM_RES[1]):
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        self.running = False
        self.paused_event = threading.Event()  # set -> paused
    ...
    def build_left(self):
        ...
        for idx, cam in enumerate(CAM_INDEXES):
            ...
            chk = ctk.CTkCheckBox(cam_sel, text=f"Cam {cam}", variable=var, command=self._on_camera_toggle)
    ...
    def build_right(self):
        side = ctk.CTkFrame(self, width=320, corner_radius=12)
        ...
        ctk.CTkLabel(side, text="Averages every 10 m\n(TOF @100 Hz)", justify="left").grid(row=12, column=0, padx=12, pady=(0,12), sticky="w")
    ...
    def _on_camera_toggle(self):
        self.camera_enabled = [var.get() for var in self.cam_checks]
    ...
    def on_start(self):
        ...
        # Start selected cameras + writers
        ...
        for enabled, cam_idx in zip(self.camera_enabled, CAM_INDEXES):
            if not enabled:
                continue
            temp_path = os.path.join(tempfile.gettempdir(), f"nsv_run_cam{cam_idx}_{int(time.time())}.mp4")
            ...
            cw = CameraWorker(cam_idx, self.cam_queue, self.paused_event, self.get_writer_for)
            self.cams.append(cw)
            cw.start()
    ...
    def on_reset(self):
        # Clear table and reset sensor worker bin state
    ...
    def on_export(self):
        ...
        try:
            ...
            # Copy videos next to the data file
            out_dir = os.path.dirname(fpath)
            base = os.path.splitext(os.path.basename(fpath))[0]
            copied_any = False
            for cam_idx, temp_path in self.video_temp_paths.items():
                if cam_idx not in self.video_writers:
                    continue
                # Ensure writer is flushed
                self.video_writers[cam_idx].release()
