"""
NSV Controller v11 — cleaned & tightened (warnings-fixed)
Fixes:
- Camera backend fallback order (MSMF → DSHOW → ANY) to avoid DSHOW index warnings.
- Graceful handling of camera index out of range; no spammy errors.
- Use CTkImage instead of PhotoImage for previews (no CTkLabel HiDPI warning).
"""
    ...
    def run(self):
        try:
            if not self._open_cap():
                # No camera at this index; just emit a testcard quietly
...
class NSVApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("NSV Controller v11 (warnings-fixed)")
    ...
    def poll_cam_frames(self):
        try:
            ...
            while True:
                ...
                # Use CTkImage to avoid HiDPI scaling warnings
