...
- Header columns:
  ...
  - Col2: Latitude, Longitude, Altitude, Dist/Chain/Speed (single row), LRP (xx.xxx Km)
...
- Right panel shows Speed, Distance, Chainage (Km), Latitude, Longitude, and Altitude.
- 6 sensors selected by default, up to 13 supported.
- Project setup fields pre-filled, including Survey Location ("Hyderabad") and LRP ("00.000").
            ...
            if not self._open_cap():
                while not self.stop_event.is_set():
                    if self.out_queue.qsize() >= MAX_QUEUE_SIZE:
                        time.sleep(0.1)  # Prevent tight loop when queue is full
                        continue
                    ...
                    self.latest_preview_frame = cv2.cvtColor(preview, cv2.COLOR_BGR2RGB)
                    self.latest_record_frame = record
                    # Only send preview frame if enough time has elapsed
                    if time.time() - self.last_frame_time >= self.frame_interval:
                        if self.out_queue.qsize() < MAX_QUEUE_SIZE:
                            self.out_queue.put((self.cam_id, self.latest_preview_frame))
                            self.last_frame_time = time.time()
                        else:
                            logger.warning(f"Camera {self.cam_id} queue full, skipping frame")
